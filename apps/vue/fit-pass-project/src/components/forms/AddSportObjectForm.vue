    ...
    <div class="wrapper">
        <form action="" class="form-wrapper">
            ...
            <div class="latitude-wrapper">
                ...
                <span class="error"> {{errors.latitudeError}}</span>

            </div>
            <div class="longitude-wrapper">
                <input v-model="longitude" type="text" placeholder="longitude">
                <span class="error"> {{errors.longitudeError}}</span>
            ...
            <div class="manager-wrapper">
                <select name="" id="" v-model="manager">
                    <option :value="manager.username" v-for="manager in managers" :key="manager.username">{{manager.name + " " + manager.surname}}</option>
                </select>
                <img src="../../assets/imgs/add-comment.png" alt="" height="25" width="25" v-show="managers.length == 0"  @click="displayRegisterManager=true;">
        ...
        <NewProfileForm v-show="displayRegisterManager" @closeRegisterForm="displayRegisterManager = false" @user-registered="registerUser"/>
    </div>
...
export default {
  ...
  methods: {
    ...
    validate() {
      this.required("name")
      this.required("type")
      this.required("longitude")
      this.required("latitude")
      this.notNull("pic")
...
.name-wrapper, .type-wrapper, .latitude-wrapper,.longitude-wrapper, .logo-wrapper, .manager-wrapper
{
    display: flex;
    flex-direction: column;
    align-items: center;
...
.street, .number, .city, .country, .postal-code{
    display: flex;
    min-height: 40px;
    border-radius: 5px;
...
.name-wrapper input, .type-wrapper select, .logo-wrapper input,
.latitude-wrapper input, .longitude-wrapper input {
    padding: 0px 0px 0px 5px;
}


.name-wrapper input, .type-wrapper select, .logo-wrapper input,
.latitude-wrapper input, .longitude-wrapper input, .manager-wrapper select{
    display: flex;
    justify-self: center;
    min-height: 40px;
    width: 80%;
...
.manager-wrapper select,  .type-wrapper select {
    width: calc(80% + 5px) 
}
